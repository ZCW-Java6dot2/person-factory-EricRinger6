...
public final class PersonWarehouse implements Iterable<Person> {
    ...
    public Stream<Person> getUniquelyNamedPeopleStartingWith(Character character) {
        List<String> list = new ArrayList<>();
        return people.stream().filter(person -> {
            if(list.contains(person.getName()) || person.getName().charAt(0) != character){
                return false;
            } else{
                list.add(person.getName());
                return true;
    ...
    public Stream<Person> getFirstNUniquelyNamedPeople(int n) {
        List<String> list = new ArrayList<>();
        return people.stream().filter(person -> {
            if(list.contains(person.getName()) || list.size() >= n){
                return false;
            } else{
                list.add(person.getName());
                return true;
    ...
    public Stream<String> getAllAliases() {
        return people.stream().map(Person::getAliases).map(Objects::toString);
    }
